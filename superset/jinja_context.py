...
from typing import Any, Callable, cast, Dict, List, Optional, Tuple, TYPE_CHECKING, Set
...
class ExtraCache:
    ...
    def __init__(self, extra_cache_keys: Optional[List[Any]] = None,
            removed_filters: Optional[Set[str]] = None
        ):
        self.extra_cache_keys = extra_cache_keys
        self.removed_filters = removed_filters if removed_filters is not None else set()
    ...
    def get_filters(self, column: str, remove_filter: bool = False) -> List[str]:
        ...
        for flt in form_data.get("adhoc_filters", []):
            ...
            if (
                ...
                if remove_filter:
                    # Adding column name to set. The set update function takes
                    # tuples, lists, dictionaries. We thus wrap it in a list.
                    self.removed_filters.update([column])
                if (
                    op in ( FilterOperator.IN.value,
